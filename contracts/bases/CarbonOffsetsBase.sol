...
abstract contract CarbonOffsetsBase is
    ERC20Upgradeable,
    CarbonOffsetsStorage
{
    // ----------------------------------------
    //              Constants
    // ----------------------------------------
    ...
    /// @dev modifier checks whether the `CarbonOffsetsFactory` is paused
    /// Since TCO2 contracts are permissionless, pausing does not function individually
    modifier whenNotPaused() {
        address tco2Factory = IContractRegistry(contractRegistry)
            .CarbonOffsetsFactoryAddress(standardRegistry());
    ...
    modifier onlyBridges() {
        address tco2Factory = IContractRegistry(contractRegistry)
            .CarbonOffsetsFactoryAddress(standardRegistry());
        bool isAllowed = ICarbonOffsetsFactory(tco2Factory)
    ...
    modifier onlyFactoryOwner() {
        address tco2Factory = IContractRegistry(contractRegistry)
            .CarbonOffsetsFactoryAddress(standardRegistry());
        address owner = ICarbonOffsetsFactory(tco2Factory).owner();
    ...
    modifier onlyEscrow() {
        require(
            IContractRegistry(contractRegistry)
                .CarbonOffsetsEscrowAddress() == msg.sender,
            'Not escrow contract'
        );
        _;
    }
    ...
    function _retire(
        ...
        address certAddr = IContractRegistry(contractRegistry)
