...
import '../cross-chain/interfaces/ICrosschainMessenger.sol';
import '../interfaces/IPoolFilter.sol';
import '../interfaces/ICarbonOffsets.sol';
...
/// It is possible to whitelist  Protocol external tokenized carbon
    ...
    BaseCarbonTonneStorage
{
    ...
    function initialize() external virtual initializer {
        ...
        __ERC20_init_unchained(' Protocol: Base Carbon Tonne', 'BCT');
    ...
    /// @param _router address of CrosschainMessenger
    ...
    /// @dev invoked only by the CrosschainMessenger (Router)
    ...
    function bridgeTCO2s(
        ...
        // Read the address of the remote pool from CrosschainMessenger
        // and set that as a recipient in our cross-chain messages.
        address tcm = router;
        RemoteTokenInformation memory remoteInfo = ICrosschainMessenger(
            tcm
        ...
        for (uint256 i; i < tco2Length; ++i) {
            ICrosschainMessenger(tcm).sendMessageWithRecipient(
                destinationDomain,
                tco2s[i],
                amounts[i],
                recipient
        ...
        )
    {
        ...
        for (uint256 i; i < tco2Length; ) {
            ...
            // on behalf of them
            if (toRetire) {
                retirementIds[i] = ICarbonOffsets(tco2s[i]).retireFrom(
                    msg.sender,
                    amountToRedeem
                );
            }
