    ...
    PoolFilterStorage
{
    ...
    function checkEligible(address erc20Addr)
        ...
        returns (bool)
    {
        bool isContract = IContractRegistry(contractRegistry)
        ...
        if (isContract) {
            ...
            /// @dev If not  native contract, check if address is whitelisted
        ...
        returns (bool)
    {
        ...
        (projectData, vintageData) = ICarbonOffsets(erc20Addr)
            .getAttributes();
    ...
    function setContractRegistry(address _address) external virtual {
        onlyPoolOwner();
