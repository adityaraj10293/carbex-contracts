...
import './ContractRegistryStorage.sol';

/// @dev The ContractRegistry is queried by other contracts for current addresses
contract ContractRegistry is
    ContractRegistryStorageLegacy,
    OwnableUpgradeable,
    ...
    ContractRegistryStorage
{
    ...
    function pauseSystem() external onlyPausers {
        ...
        for (uint256 i; i < standardRegistriesLen; ) {
            ...
            address factory = CarbonOffsetFactories[standardRegistry];
    ...
    function setCarbonOffsetsFactoryAddress(address tco2Factory)
        external
        virtual
        onlyOwner
    {
        ...
        string memory standardRegistry = ICarbonOffsetsFactory(
            tco2Factory
        ).standardRegistry();
        ...
        CarbonOffsetFactories[standardRegistry] = tco2Factory;
    ...
    function setCarbonOffsetsEscrowAddress(address _address)
        external
        virtual
        onlyOwner
    {
        require(_address != address(0), 'Zero address');
        _CarbonOffsetsEscrowAddress = _address;
    }
    ...
    function addERC20(address erc20)
        external
        virtual
        onlyBy(DEPRECATED_CarbonOffsetsFactoryAddress, owner())
    {
    ...
    function addERC20(address erc20, string calldata standardRegistry)
        external
        virtual
        onlyBy(CarbonOffsetFactories[standardRegistry], owner())
    {
        projectVintageERC20Registry[erc20] = true;
    }
    ...
    /// TODO: Kept for backwards-compatibility; will be removed in a future
    /// upgrade in favor of CarbonOffsetsFactory('verra')
    function CarbonOffsetsFactoryAddress()
        ...
        returns (address)
    {
        return DEPRECATED_CarbonOffsetsFactoryAddress;
    }
    ...
    function CarbonOffsetsFactoryAddress(string memory standardRegistry)
        ...
        returns (address)
    {
        return CarbonOffsetFactories[standardRegistry];
    }

    function CarbonOffsetsEscrowAddress()
        external
        view
        virtual
        override
        returns (address)
    {
        return _CarbonOffsetsEscrowAddress;
    }
